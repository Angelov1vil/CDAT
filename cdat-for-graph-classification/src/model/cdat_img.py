...
from .core import CDATLayerNorm, HopfieldTransformer
...
class CDAT(nn.Module):
    ...
    def setup(self):
        ...
        self.encode_norm = [
            CDATLayerNorm(self.embed_dim, dtype=self.dtype, eps=self.eps)
            for _ in range(self.block)
        ]
        ...
        self.P = self.param('P', nn.initializers.lecun_normal(), (self.r, self.num_tokens+1))
        self.Q = self.param('Q', nn.initializers.lecun_normal(), (self.r, self.r))
    ...
    # def suppression(self, x, grad=None):
    #     # grad is the gradient of e with respect to g, default to 1 if not provided
    #     if grad is None:
    #         grad_norm = 1.0
    #     else:
    #         grad_norm = jnp.linalg.norm(grad)
    #     # Normalize gradient to prevent extreme values
    #     grad_norm = jnp.clip(grad_norm, 0.0, 100.0)
    #     # Compute c: smaller gradient leads to smaller c, and c >= 0.1
    #     c = jnp.maximum(1.0 - grad_norm / 100.0, 0.1)
    #     return c * x

    # def suppression(self, x, t, base=1.0, gamma=0.03, min_c=1):
    #     # c decreases exponentially with time step t
    #     c = jnp.maximum(base * jnp.exp(gamma * t), min_c)
    #     return c * x
    def suppression(self, x):
        # c decreases exponentially with time step t
        c = self.c 
    ...
    def encode_features(
        ...
        """Pass tokens into the CDAT"""
        embeddings, energies = [], []
        P_slice = self.P[:, :x.shape[0]]
        x0 = x
        ...
        for i in range(self.block):
            for t in range(self.depth):
                ...
                e, grad = self.encode_block[i](x, corr)
                ...
                x = x - self.alpha * grad -self.alpha * (x+self.suppression(x)) + self.alpha * (P_slice.T @ self.Q @ P_slice @ x) + (self.alpha**0.5) * noise
