    ...
    def encode_features(
        ...
        for i in range(self.block):
            for t in range(self.depth):
                ...
                x = x - self.alpha * grad - self.alpha * (x + self.suppression_scale * self.suppression(x)) + self.alpha * (P_slice.T @ self.Q @ P_slice @ x) + self.noise_scale * (self.alpha**0.5) * noise
                # x = x - self.alpha * grad + self.alpha * (P_slice.T @ self.Q @ P_slice @ x) + (self.alpha**0.5) * noise
                # x = x - self.alpha * grad -self.alpha * (x+self.suppression(x))  + (self.alpha**0.5) * noise
